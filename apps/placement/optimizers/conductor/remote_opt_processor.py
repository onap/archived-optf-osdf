...
def process_placement_opt(request_json, policies, osdf_config):
    ...
    try:
        ...
        if request_json.get('placementInfo', {}).get('placementDemands'):
            ...
            resp, raw_resp = conductor.request(req_info, demands, request_parameters, service_info,
                                               osdf_config, policies)
            if resp["plans"][0].get("recommendations"):
                placement_response = conductor_response_processor(resp, raw_resp, req_id)
            else:  # "solved" but no solutions found
                placement_response = conductor_no_solution_processor(resp, raw_resp, req_id)
