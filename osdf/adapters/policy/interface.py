...
def get_by_scope(rest_client, req, config_local, type_service):
    ...
    """
    policy_list = []
    references = config_local['references']
    pscope = config_local['policy_info'][type_service]['policy_scope']
    service_name = dot_notation(req, references['service_name']['value'])
    primary_scope = pscope['{}_scope'.format(service_name.lower() if service_name else "default")]
    for sec_scope in pscope['secondary_scopes']:
...
def get_scope_fields(field, references, req, policy_info):
    ...
    """
    if references[field['get_param']]['source'] == "request":
        scope_field = dot_notation(req, references[field['get_param']]['value'])
        if scope_field:
            return scope_field
        raise BusinessException("Field %s is missing a value in a request"
                                % references[field['get_param']]['value'].split('.')[-1])
    else:
        scope_fields = []
        for policy in policy_info:
            policy_content = json.loads(policy['config'])
            if policy_content['content']['policyType'] == references[field['get_param']]['source']:
                scope_fields.append(dot_notation(policy_content, references[field['get_param']]['value']))
        scope_values = list_flatten(scope_fields)
        if len(scope_values) > 0:
            return scope_values
        raise BusinessException("Field %s is missing a value in all policies of type"
                                % references[field['get_param']]['value'].split('.')[-1],
                                references[field['get_param']]['source'])
