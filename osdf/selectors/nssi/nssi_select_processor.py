...
from osdf.models.api.nssiSelectionResponse import NSSISelectionResponse, NSSIInfo
...
def process_nssi_selection(request_json, osdf_config, flat_policies):
    """
    ...
    try:
        ...
        status, nssi_selector = build_solution_list(request_json, osdf_config, flat_policies)
        nssi_response = get_solutions(request_json, status, nssi_selector, None)
        metrics_log.info(MH.inside_worker_thread(req_id))
        try:
            NSSISelectionResponse(nssi_response).validate()
        ...
        nssi_response = get_solutions(request_json, "failed", "", error_msg)
        metrics_log.info(MH.sending_response(req_id, "ERROR"))

    return nssi_response


def get_solutions(request_json, status, nssi_selector, errormsg):
    nssi_resp = NSSISelectionResponse()
    nssi_resp.requestId = request_json["requestInfo"]["requestId"]
    nssi_resp.transactionId = request_json['requestInfo']['transactionId']
    nssi_resp.requestStatus = status
    if status == "success":
        # if nssi_selector content not empty
        if nssi_selector:
            nssi_resp.NSSIInfo = nssi_selector
            nssi_resp.statusMessage = ""
        else:
            nssi_resp.statusMessage = "Did not find any nssi solution."
        # return {
        ...
        # }
        return nssi_resp.serialize()
    else:
        nssi_resp.statusMessage = errormsg
        # return {
        #    "transactionId": request_json['requestInfo']['transactionId'],
        #    "requestId": request_json["requestInfo"]["requestId"],
        #    "requestStatus": status,
...
def build_solution_list(request_json, osdf_config, flat_policies):
    ...
    try:
        ...
        # nssi_selector = build_nssi_selector()
        # return from remote rest call
        nssi_selector = nsconfig_request(request_json, osdf_config, flat_policies, 'NSSI')
    except RuntimeError:
        error_log.error("Failed finding solution for {} {}".format(req_id, traceback.format_exc()))
        status = "failed"
    return status, nssi_selector


def build_nssi_selector():
    nssiselector = NSSIInfo()
    nssiselector.invariantUUID = "23edd22b-a0b2-449f-be87-d094159b9269"
    nssiselector.UUID = "46da8cf8-0878-48ac-bea3-f2200959411a"
    nssiselector.NSSIID = "61c4aa7c-9ca6-46cd-b3f8-009769db8641"
    nssiselector.NSSIName = "eMBB cn service"
    return nssiselector
