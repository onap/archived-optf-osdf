...
from osdf.models.api.nsiSelectionResponse import NSISelectionResponse, NSIInfo
...
def process_nsi_selection(request_json, osdf_config, flat_policies):
    """
    ...
    try:
        ...
        status, nsi_selector = build_solution_list(request_json, osdf_config, flat_policies)
        nsi_response = get_solutions(request_json, status, nsi_selector, None)
        metrics_log.info(MH.inside_worker_thread(req_id))
        try:
            NSISelectionResponse(nsi_response).validate()
        ...
        nsi_response = get_solutions(request_json, "failed", "", error_msg)
        metrics_log.info(MH.sending_response(req_id, "ERROR"))

    return nsi_response


def get_solutions(request_json, status, nsi_selector, errormsg):
    nsi_resp = NSISelectionResponse()
    nsi_resp.requestId = request_json["requestInfo"]["requestId"]
    nsi_resp.transactionId = request_json['requestInfo']['transactionId']
    nsi_resp.requestStatus = status
    if status == "success":
        # if not empty
        if nsi_selector:
            nsi_resp.NSIInfo = nsi_selector
            nsi_resp.statusMessage = ""
        else:
            nsi_resp.statusMessage = "Did not find any nsi solution."
        # return {
        ...
        # }
        return nsi_resp.serialize()
    else:
        nsi_resp.statusMessage = errormsg
        # return {
        #    "transactionId": request_json['requestInfo']['transactionId'],
        #    "requestId": request_json["requestInfo"]["requestId"],
        #    "requestStatus": status,
...
def build_solution_list(request_json, osdf_config, flat_policies):
    ...
    try:
        ...
        #nsi_selector = build_nsi_selector()
        # return from remote rest call
        nsi_selector = nsconfig_request(request_json, osdf_config, flat_policies, 'NSI')
    except RuntimeError:
        error_log.error("Failed finding solution for {} {}".format(req_id, traceback.format_exc()))
        status = "failed"
    return status, nsi_selector


def build_nsi_selector():
    nsiselector = NSIInfo()
    nsiselector.invariantUUID = "23edd22b-a0b2-449f-be87-d094159b9269"
    nsiselector.UUID = "46da8cf8-0878-48ac-bea3-f2200959411a"
    nsiselector.NSIID = "61c4aa7c-9ca6-46cd-b3f8-009769db8641"
    nsiselector.NSIName = "eMBB service"
    return nsiselector
