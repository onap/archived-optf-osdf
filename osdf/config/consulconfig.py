...
class Config(object):
    def __init__(self, loop, osdf_final_config):
        ...
        self.c=Consul(host=osdf_config['consulHost'],port=osdf_config['consulPort'],scheme=osdf_config['consulScheme'],
                             verify=osdf_config['consulVerify'],cert=osdf_config['consulCert'])
        self.result = json.dumps(self.config)
        self.c.kv.put("osdfconfiguration",self.result)
    ...
    def watch(self):
        ...
        while True:
            try:
                index, data = yield self.c.kv.get('osdfconfiguration', index=index)
                if data is not None:
